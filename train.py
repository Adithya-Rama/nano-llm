...
Colab-resilient features:
  - Time-based checkpointing (every ckpt_interval_secs seconds)
  - SIGTERM emergency handler (saves on Colab preemption)
  - Atomic checkpoint writes (no corruption from partial writes)
  - GradScaler state saved for float16 resume
  - JSONL structured training logs for plotting
"""
...
# Modern architecture options (default False = vanilla nanoGPT)
use_rmsnorm = False  # RMSNorm instead of LayerNorm
use_rope = False     # Rotary positional embeddings instead of learned wpe
use_swiglu = False   # SwiGLU FFN instead of GELU MLP
use_qk_norm = False  # QK-Norm: RMSNorm on Q/K before attention (Gemma 2)
# Training enhancements
use_gradient_checkpointing = False  # Trade compute for memory
label_smoothing = 0.0  # Label smoothing factor (0.0 = none, 0.1 = typical)
# adamw optimizer
...
iter_num = 0
best_val_loss = 1e9
train_time_total = 0.0  # cumulative training time across sessions
...
model_args = dict(n_layer=n_layer, n_head=n_head, n_embd=n_embd, block_size=block_size,
                  ...
                  use_gradient_checkpointing=use_gradient_checkpointing) # start with model_args from command line
if init_from == 'scratch':
    ...
    for k in ['n_layer', 'n_head', 'n_embd', 'block_size', 'bias', 'vocab_size',
              'use_rmsnorm', 'use_rope', 'use_swiglu', 'use_qk_norm',
              'use_gradient_checkpointing']:
        model_args[k] = checkpoint_model_args.get(k, model_args.get(k, False))
    # create the model
    gptconf = GPTConfig(**model_args)
    model = GPT(gptconf)
    ...
    train_time_total = checkpoint.get('train_time_total', 0.0)
...
def _sigterm_handler(signum, frame):
    """Emergency checkpoint on Colab preemption (SIGTERM)."""
...
# Only install handler on Linux (Colab) — Windows doesn't support SIGTERM well
...
# JSONL training log
# ---------------------------------------------------------------------------
def log_training_step(step, loss_val, val_loss=None, lr_val=None, mfu_val=None, dt_val=None):
    """Append a structured log entry to train_log.jsonl."""
    ...
    if mfu_val is not None:
        entry["mfu"] = round(mfu_val, 4)
    if dt_val is not None:
        entry["dt_ms"] = round(dt_val * 1000, 2)
    log_path = os.path.join(out_dir, 'train_log.jsonl')
    try:
        with open(log_path, 'a') as f:
            f.write(json.dumps(entry) + '\n')
    except Exception:
        pass  # don't crash training if log write fails
...
while True:
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        # Log training step (only every log_interval to avoid huge log files)
        log_training_step(iter_num, lossf, lr_val=lr, mfu_val=running_mfu, dt_val=dt)

    # Time-based checkpoint (Colab resilience)
    if ckpt_interval_secs > 0 and master_process:
        if time.time() - last_ckpt_time > ckpt_interval_secs:
            print(f"⏰ Time-based checkpoint (every {ckpt_interval_secs}s)")
            save_checkpoint()
            last_ckpt_time = time.time()
